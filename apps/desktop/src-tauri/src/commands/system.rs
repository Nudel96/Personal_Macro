...
pub async fn save_account(
    ...
    sqlx::query_as::<_, Account>("SELECT a.id, a.name, a.broker, a.account_type, a.base_currency, a.initial_balance_minor, a.default_risk_percent, a.is_archived, COALESCE((SELECT c.balance_minor FROM broker_account_connections c WHERE c.local_account_id = a.id AND c.balance_minor IS NOT NULL LIMIT 1), a.initial_balance_minor + COALESCE((SELECT SUM(ac.amount_minor) FROM account_cashflows ac WHERE ac.account_id = a.id), 0) + COALESCE((SELECT SUM(t.net_pnl_minor) FROM trades t WHERE t.account_id = a.id AND t.status = 'closed' AND t.is_deleted = 0), 0)) AS current_balance_minor FROM accounts a WHERE a.id = ?")
